...
if __name__ == "__main__":
    ...
    dois_info = pd.read_csv(f"dois_info.csv", sep=";")[lambda x: (x['pubyear'] > 2015) & (x['pubyear'] < 2024)].copy()
    economic_dois = dois_info[dois_info['category'] == 'economic']['doi'].unique()
    social_dois = dois_info[dois_info['category'] == 'social']['doi'].unique()
    # Count the number of unique DOIs that appear in both categories
    unique_dois_count = len(set(economic_dois) & set(social_dois))
    print("all dois in social:",dois_info[dois_info['category'] == 'social']['doi'].nunique())
    print("all dois in economic:",dois_info[dois_info['category'] == 'economic']['doi'].nunique())
    print("overlap:",unique_dois_count)
  
    #dois = dois_info['doi'].unique()
    #get_domain(dois).to_csv(f"dois_domains.csv", index=False)
    dois_domains_all = pd.read_csv(f"dois_domains.csv", sep=";",dtype={'status': 'object'})
    dois_domains =dois_domains_all.merge(dois_info,on=('doi'),how='inner').drop_duplicates()[['doi', 'domain', 'status']].reset_index(drop=True)
    ...
    for category in categories:    
        ...
        # Calculate h-index 
        #dois_all_info = dois_all_info[(dois_all_info['pubyear'] > 2015) & (dois_all_info['pubyear'] < 2024)]
        #&(dois_all_info['domain']!='https://primarysources.brillonline.com/') & (dois_all_info['domain']!='https://referenceworks.brillonline.com/')]
        

        domain_h_index = dois_all_info.groupby(['domain', 'category']).apply(calculate_h_index).reset_index()
        ...
        domain_doi_count_visibility_hindex=domain_doi_count_visibility_hindex[columns_to_keep].drop_duplicates()
        ...
        std_dev_cite_norm[category] = dois_all_info[dois_all_info['category'] == category]['cite_norm'].std()
        mean_cite_norm[category] = dois_all_info[dois_all_info['category'] == category]['cite_norm'].mean()
        ...
        for threshold in thresholds:
            dois_all_info_hindex = dois_all_info.merge(domain_h_index,on = 
            ['domain', 'category'], how = 'inner')[['doi','domain','category','cit_all_years','visibility_index','cite_norm','h_index']].drop_duplicates()
            #dois_all_info_hindex = dois_all_info_hindex[(dois_all_info_hindex['domain']!='https://referenceworks.brillonline.com/') & 
            #(dois_all_info_hindex['domain']!='https://www.oecd-ilibrary.org/')].reset_index()
            dois_all_info_cat = dois_all_info_hindex[dois_all_info_hindex['category'] == category]
            dois_all_info_filtered_cat = dois_all_info_cat[dois_all_info_cat['cite_norm'] > threshold]
            spearman_corr = stats.spearmanr(dois_all_info_filtered_cat['visibility_index'], dois_all_info_filtered_cat['cite_norm'])
            ...
            num_domains = len(dois_all_info_filtered_cat)
    ...
    for category in categories:
        ...
        for threshold in thresholds:
            ...
            if threshold == -1:
                ...
                threshold_label = "Datasets with number of citations $>$ 0"
            elif threshold == 1:
                threshold_label = "Datasets with number od citations $>$ 1"
            '''
    ...
    # Save the LaTeX table into a file
    with open('latex_table_statistic.tex', 'w') as f:
        f.write(latex_table)
